...
// Define connectivity options for dropdown
const connectivityOptions = ["Column Flange-Beam-Web", "Column Web-Beam-Web", "Beam-Beam"]
...
const MenuItems = [
  ...
  {
    label: "Database",
    dropdown: [{ name: "Downloads", options: ["Column", "Beam", "Angle", "Channel"] }, { name: "Reset" }],
  },
  {
    label: "Help",
    dropdown: [
...
// Add this to your component's style section or CSS file
const beamOptionStyles = {
  beamOptions: {
    padding: "8px",
...
function EndPlate() {
  ...
  }, [location.pathname, deleteSession])
  ...
  useEffect(() => {
    ...
    if (selectedOption === "Column Flange-Beam-Web") {
      ...
      setImageSource(BB)
      // Reset beam selections when switching to Beam-Beam
      if (inputs.primary_beam === undefined || inputs.secondary_beam === undefined) {
        setInputs((prev) => ({
          ...prev,
          primary_beam: "Select Section",
          secondary_beam: "Select Section",
        }))
      }
    } else {
      setImageSource(ErrorImg)
  ...
  useEffect(() => {
    if (renderCadModel !== undefined) {
      setRenderBoolean(renderCadModel)
    }
  }, [renderCadModel])
  ...
  useEffect(() => {
    const connectivity = conn_map[selectedOption]
    if (connectivity === "Column Flange-Beam Web" || connectivity === "Column Web-Beam Web") {
      if (inputs.column_section && inputs.beam_section) {
        getDesingPrefData({
          supported_section: inputs.beam_section,
          supporting_section: inputs.column_section,
          connectivity: connectivity.split(" ").join("-"),
        })
      }
  ...
  const handleSubmit = async () => {
    const connectivity = conn_map[selectedOption]
    const isColumnBeam = connectivity === "Column Flange-Beam Web" || connectivity === "Column Web-Beam Web"
    ...
    if (!isColumnBeam && (!inputs.primary_beam || !inputs.secondary_beam)) {
      alert("Please input all the fields")
      return
    }
    ...
    const param = {
      ...
      "Load.Axial": inputs.load_axial || "",
      "Load.Shear": inputs.load_shear || "",
      Material: isColumnBeam ? "E 250 (Fe 410 W)A" : "E 300 (Fe 440)",
      "Member.Supported_Section.Designation": isColumnBeam ? inputs.beam_section : inputs.secondary_beam,
      "Member.Supported_Section.Material": inputs.supported_material,
      "Member.Supporting_Section.Designation": isColumnBeam ? inputs.column_section : inputs.primary_beam,
      "Member.Supporting_Section.Material": inputs.supporting_material,
      Module: "End Plate Connection",
      "Weld.Fab": inputs.weld_fab,
      "Weld.Material_Grade_OverWrite": inputs.weld_material_grade,
  ...
  const handleReset = () => {
    ...
    handleSelectChangePropertyClass("All")
    handleSelectChangeBoltBeam("All")
    setRenderBoolean(false)
    setOutput(null)
  }
  ...
  const saveOutput = () => {
    ...
    const data = {
      ...inputs,
      ...Object.entries(output).reduce((acc, [key, value]) => {
        value.forEach(({ label, val }) => {
          acc[`${key}.${label.split(" ").join("_")}`] = val
        })
        return acc
      }, {}),
    }
  ...
  return (
    <div style={{ width: "100%" }}>
      <div className="module_nav">
        ...
        <h1 className="element">
          <Button onClick={() => navigate("/home")} style={{ backgroundColor: "black", color: "white" }}>
            Home
          </Button>
        </h1>
      </div>
      ...
      <div className="superMainBody">
        {/* Left Panel */}
        <div>
          <div className="component-grid"></div>
          <h5>Input Dock</h5>
          <div className="subMainBody scroll-data">
            ...
            <div className="component-grid">
              ...
              <div>
                <Select style={{ width: "100%" }} onSelect={handleSelectChange} value={selectedOption}>
              ...
              <div>
                <img src={imageSource || "/placeholder.svg"} alt="Component" height="100px" width="100px" />
              </div>
              ...
              {selectedOption === "Beam-Beam" ? (
                <>
                  ...
                  <div>
                    <Select
                      style={{ width: "100%" }}
                      value={inputs.primary_beam || "Select Section"}
                      onSelect={(value) => setInputs({ ...inputs, primary_beam: value })}
                      dropdownRender={(menu) => (
                        <div>
                          {menu}
                          <div className="beam-options">
                            <div className="beam-option">JB 150</div>
                            <div className="beam-option">JB 175</div>
                            <div className="beam-option">JB 200</div>
                            <div className="beam-option">JB 225</div>
                          </div>
                        </div>
                      )}
                    >
                      ...
                      {beamList
                        ?.filter((item) => !["JB 150", "JB 175", "JB 200", "JB 225"].includes(item))
                        .map((item, index) => (
                          <Option key={index} value={item}>
                            {item}
                          </Option>
                  ...
                  <div>
                    <Select
                      style={{ width: "100%" }}
                      value={inputs.secondary_beam || "Select Section"}
                      onSelect={(value) => setInputs({ ...inputs, secondary_beam: value })}
                      dropdownRender={(menu) => (
                        <div>
                          {menu}
            ...
            <div className="component-grid">
              ...
              <div>
                <Input
                  ...
                  value={inputs.load_shear}
                  onChange={(e) => setInputs({ ...inputs, load_shear: e.target.value })}
                />
              </div>
              ...
              <div>
                <Input
                  ...
                  value={inputs.load_axial}
                  onChange={(e) => setInputs({ ...inputs, load_axial: e.target.value })}
                />
              </div>
            </div>
            ...
            <div className="component-grid">
              ...
              <div>
                <Select style={{ width: "100%" }} onSelect={handleSelectChangeBoltBeam} value={boltDiameterSelect}>
                  <Option value="Customized">Customized</Option>
                  <Option value="All">All</Option>
                </Select>
              </div>
        ...
        <div className="superMainBody_mid">
          {renderBoolean ? (
            ...
            <div style={{ maxWidth: "740px", height: "600px", border: "1px solid black" }}>
              <img src={cad_background || "/placeholder.svg"} alt="Demo" height="100%" width="100%" />
            </div>
          )}
          <br />
          <div>
      ...
      <Modal open={plateThicknessModal} onCancel={() => setPlateThicknessModal(false)} footer={null} width={700}>
        <div style={{ display: "flex" }}>
          <div style={{ marginRight: "20px" }}>
            <h3>Customized</h3>
            <Transfer
      ...
      <Modal
        open={CreateDesignReportBool}
        onCancel={() => setCreateDesignReportBool(false)}
        footer={null}
        style={{ border: "1px solid #ccc" }}
        bodyStyle={{ padding: "20px" }}
      >
